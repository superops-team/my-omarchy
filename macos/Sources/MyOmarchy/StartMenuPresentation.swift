...
enum StartMenuPresentation {
    ...
    static let bootRecoveryConfirmationDetail = """
        My Omarchy found an existing VM from an earlier app version. Before it starts, My Omarchy will run a one-time, read-only recovery to pair that VM with its own kernel and startup files. The saved disk and all of its data remain intact.

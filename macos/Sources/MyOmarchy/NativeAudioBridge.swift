...
final class CoreAudioDeviceChangeMonitor {
    private let queue = DispatchQueue(label: "team.superops.myomarchy.audio-devices")
...
final class NativeAudioBridge {
    ...
    private let stateQueue = DispatchQueue(label: "team.superops.myomarchy.audio-bridge-state")
    private let writeQueue = DispatchQueue(label: "team.superops.myomarchy.audio-bridge-writes")

    ...
    private static func recordedPersistentDisk(
        ...
        guard hasAttributes(
            ...
            fileManager: fileManager
              ...
              metadata["kind"] as? String == "my-omarchy-persistent-disk",
        ...
        let canonicalMetadata = "{\"bundleIdentity\":\"\(identity)\",\"kind\":\"my-omarchy-persistent-disk\",\"schemaVersion\":\(schemaNumber.intValue),\"sourceRootfs\":{\"bytes\":\(sourceBytesNumber.int64Value),\"sha256\":\"\(sourceSHA)\"}}"
...
enum QEMUGPULauncherPath {
    static let appName = "My Omarchy.app"
...
struct MicrophoneLaunchDecision: Equatable {
    ...
    static func make(for state: MicrophoneAuthorizationState) -> Self {
        ...
        case .denied:
            Self(
                allowsLaunch: true,
                warning: "Microphone access is denied. Audio playback will continue, but guest recording is unavailable. Enable My Omarchy in System Settings > Privacy & Security > Microphone, then relaunch."
            )
        case .restricted:
            Self(
                allowsLaunch: true,
                warning: "Microphone access is restricted by macOS policy. Audio playback will continue, but guest recording is unavailable. Ask the Mac administrator to allow microphone access for My Omarchy."
            )
        case .notDetermined:
            Self(
                allowsLaunch: true,
...
struct CameraLaunchDecision: Equatable {
    ...
    static func make(for state: CameraAuthorizationState) -> Self {
        ...
        case .denied:
            Self(
                allowsLaunch: true,
                warning: "Camera access is denied. Omarchy will continue without the Mac camera. Enable My Omarchy in System Settings > Privacy & Security > Camera, then relaunch."
            )
        case .restricted:
            Self(
                allowsLaunch: true,
                warning: "Camera access is restricted by macOS policy. Omarchy will continue without the Mac camera. Ask the Mac administrator to allow camera access for My Omarchy."
            )
        case .notDetermined:
            Self(
                allowsLaunch: true,

...
private func writeRecordedPersistentDisk(
    ...
    try Data(
        "{\"bundleIdentity\":\"\(identity)\",\"kind\":\"my-omarchy-persistent-disk\",\"schemaVersion\":\(schemaVersion),\"sourceRootfs\":{\"bytes\":1,\"sha256\":\"\(sourceSHA)\"}}\n".utf8
    ).write(to: metadata)
    try FileManager.default.setAttributes(
...
struct StorageLocationPolicyTests {
    ...
    func rejectsDamagedWorkspaceMarker(kind: String) throws {
        ...
        case "wrong content":
            try Data("my-omarchy-storage-root-v2\n".utf8).write(to: marker)
            try FileManager.default.setAttributes(
                [.posixPermissions: 0o600], ofItemAtPath: marker.path
            )
        case "wrong mode":
